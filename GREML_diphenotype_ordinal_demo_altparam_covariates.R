...
# It also demonstrates how to include covariates in an mxGREML analysis of ordinal traits.
...
y1lat <- glat[1:N] + elat[,1] + 0.2*x1 - 0.2*x2
#Trait 2, latent scale:
y2lat <- glat[(N+1):(2*N)] + elat[,2] + 0.1*x3 - 0.1*x4
...
threshmod <- mxModel(
	"Threshold_Model",
	mxData(observed=data.frame(y1=mxFactor(y1obs,levels=c(0,1,2)),y2=mxFactor(y2obs,levels=c(0,1,2)),x1=x1,x2=x2,x3=x3,x4=x4),type="raw"),
	mxMatrix(
		...
		name="Tau"),
	mxMatrix(type="Full",nrow=1,ncol=2,free=F,labels=c("data.x1","data.x2"),name="X1"),
	mxMatrix(type="Full",nrow=1,ncol=2,free=F,labels=c("data.x3","data.x4"),name="X2"),
	mxMatrix(type="Full",nrow=2,ncol=1,free=T,values=0,labels=c("b1","b2"),name="B1"),
	mxMatrix(type="Full",nrow=2,ncol=1,free=T,values=0,labels=c("b3","b4"),name="B2"),
	mxMatrix(type="Stand",nrow=2,free=T,values=cor(y1obs,y2obs,use="pair"),labels="rho",lbound=-0.9999,ubound=0.9999,name="R"),
	#Regression intercepts are zero:
	mxAlgebra(cbind(X1%*%B1, X2%*%B2), name="Mu"),
	mxFitFunctionML(),
...
lm1 <- lm(y1obs~x1+x2)
vp1.ini <- var(lm1$residuals)
yy1 <- (lm1$residuals)/sqrt(vp1.ini)
#Standardized phenotype 2:
lm2 <- lm(y2obs~x3+x4)
vp2.ini <- var(lm2$residuals)
yy2 <- (lm2$residuals)/sqrt(vp2.ini)
...
her1 <- ifelse(her1<1e-8, 1e-8, her1)
her1 <- ifelse(her1>0.9999, 0.9999, her1)
her2 <- ifelse(her2<1e-8, 1e-8, her2)
her2 <- ifelse(her2>0.9999, 0.9999, her2)
#Free some memory:
rm(U,xx,yy1,yy1.2,yy2,yy2.2,lm1,lm2); gc()


#With mxGREML, you have to treat phenotypes that are actually ordinal as though they're continuous:
widedata <- cbind(y1=as.numeric(y1obs), y2=as.numeric(y2obs), x1=x1, x2=x2, x3=x3, x4=x4)
gremldat <- mxGREMLDataHandler(data=widedata,yvars=c("y1","y2"),Xvars=list(c("x1","x2"),c("x3","x4")),blockByPheno=TRUE)
head(gremldat$yX)
...
gremlmod <- mxModel(
	...
	mxMatrix(type="Full",nrow=1,ncol=1,free=T,
					 ...
					 labels="h2lat1",lbound=1e-8,ubound=0.9999,name="H2lat1"),
	#Trait 2's latent-scale heritability:
	mxMatrix(type="Full",nrow=1,ncol=1,free=T,
					 values=her2*vp2.ini/k2,
					 labels="h2lat2",lbound=1e-8,ubound=0.9999,name="H2lat2"),
	#Conversion factors:
	...
	mxMatrix(type="Full",nrow=1,ncol=1,free=T,values=vp1.ini,labels="vp1",lbound=0.0001,name="Vp1"),
	mxMatrix(type="Full",nrow=1,ncol=1,free=T,values=vp2.ini,labels="vp2",lbound=0.0001,name="Vp2"),
	#Genetic correlation:
