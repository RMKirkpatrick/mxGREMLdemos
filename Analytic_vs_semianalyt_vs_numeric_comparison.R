...
#and once with analytic derivatives.
#The 3 sets of results are substantially equivalent.
...
# Create an MxModel that will use a compute plan and semi-analytic derivatives: #############################
...
summary(semiAnalytRun)
object.size(semiAnalytRun)
...
numericRun$output$fit; semiAnalytRun$output$fit; analyticRun$output$fit
#Point estimates agree:
coef(analyticRun)-coef(numericRun)
coef(semiAnalytRun)-coef(numericRun)
coef(analyticRun)-coef(semiAnalytRun)
cor(coef(analyticRun),coef(numericRun))
cor(coef(semiAnalytRun),coef(numericRun))
#Analytic derivatives are fastest:
numericRun$output$wallTime; semiAnalytRun$output$wallTime; analyticRun$output$wallTime
