...
# Compute plan, with Newton-Raphson:
...
m1 <- mxModel(
	...
	mxFitFunctionGREML(dV=c(ve1="dV_dve1",ve2="dV_dve2",va1="dV_dva1",va2="dV_dva2"),infoMatType="expected")
...
m2 <- mxModel(
	...
	mxAlgebra(rbind(
		cbind(V12,V12),
		cbind(V12,V12)
	),name="dV_dmu"),#<--Derivative of 'V' w/r/t any of the mean parameters.
	
	# Derivatives of 'yhat' w/r/t free parameters:
	mxAlgebra(rbind(Uno,Zip),name="dyhat_dmu1"),
	mxAlgebra(rbind(Zip,Uno),name="dyhat_dmu2"),
	mxAlgebra(rbind(Zip,Zip),name="dyhat_dV"),#<--Derivative of 'yhat' w/r/t any of the covariance parameters.
...
# In the next model, the covariance matrix is not linear in the free parameters, so we should
# not use Newton-Raphson:
plan2 <- mxComputeSequence(
	steps=list(
		mxComputeGradientDescent(engine=mxOption(NULL,"Default optimizer"),verbose=5L),
		mxComputeOnce('fitfunction', c('gradient','hessian'),verbose=5L),
		mxComputeStandardError(),
		mxComputeHessianQuality(),
		mxComputeReportDeriv(),
...
m3 <- mxModel(
	...
	mxAlgebra(rbind(
		cbind(Zilch,Zilch),
		cbind(Zilch,Zilch)
	),name="dV_dmu"),#<--Derivative of 'V' w/r/t either of the 'mu's.
	mxAlgebra(rbind(
		cbind(Zilch,V11),
		cbind(V11,V11%x%(2*B))
	),name="dV_db"),#<--Derivative of 'V' w/r/t the regression slope.
	
	# Derivatives of yhat w/r/t free parameters:
	mxAlgebra(rbind(Uno,Zip)+rbind(Zip,Uno%x%B),name="dyhat_dmu1"),
	mxAlgebra(rbind(Zip,Uno),name="dyhat_dmu2"),
	mxAlgebra(rbind(Zip,Zip),name="dyhat_dV"),#<--Derivative of 'yhat' w/r/t any of the variance components.
	
	# The GREML fitfunction...
	# Notice that parameter "b" is a dependency of both the mean and variance:
	mxFitFunctionGREML(
...
# As a bonus, let's try Model #1 again, but this time, let's use the average-information approximation: ####
m4 <- mxModel(
	"EndogenousRegression1revisited",
	plan,
	mxData(observed=widedata,type="raw",sort=F), #<--N.B. `sort=FALSE`!
	# There are no covariates for y1; y1 is a covariate for y2; we are using ordinary maximum-likelihood:
	mxExpectationGREML(V="V",yvars=c("y1","y2"),Xvars=list(character(0),c("y1")),addOnes=T,REML=F),
	...
	# The GREML fitfunction:
	mxFitFunctionGREML(dV=c(ve1="dV_dve1",ve2="dV_dve2",va1="dV_dva1",va2="dV_dva2"),infoMatType="average")
	#^^^`infoMatTrype="average"`
)
m4 <- mxRun(m4)

# Same -2logL at the solution:
m1$output$fit; m2$output$fit; m3$output$fit; m4$output$fit
# Point estimates agree:
c(coef(m1),m1$expectation$b)-c(coef(m4),m4$expectation$b)
coef(m2)-c(coef(m4),m4$expectation$b)
coef(m3)-c(coef(m4),m4$expectation$b)
coef(m3)-c(coef(m1),m1$expectation$b)
cor(c(coef(m1),m1$expectation$b),c(coef(m4),m4$expectation$b))
cor(coef(m2),c(coef(m4),m4$expectation$b))
cor(coef(m3),c(coef(m4),m4$expectation$b))
# Running times...:
m1$output$wallTime; m2$output$wallTime; m3$output$wallTime; m4$output$wallTime
#^^^As expected, Model #4 is the fastest.  It uses the faster Hessian approximation, and has only 4
#(as opposed to 7) explicit free parameters to optimize.

# Model #4 uses a different Hessian approximation, so its SEs are a tad different from the others':
cbind(
	c(m1$output$standardErrors,sqrt(diag(m1$expectation$bcov))),
	m2$output$standardErrors,
	m3$output$standardErrors,
	c(m4$output$standardErrors,sqrt(diag(m4$expectation$bcov))))
